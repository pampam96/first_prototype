...
void servo_cb( const std_msgs::Int32MultiArray& cmd_msg){
 ...
 //reading the size of the data
  sized=cmd_msg.data[0];
  datas=String(sized);
  datas.toCharArray(d,4);
  nh.loginfo(d);

   sized=cmd_msg.data[1];
  datas=String(sized);
  datas.toCharArray(d,4);
  nh.loginfo(d);

  sized=cmd_msg.data[2];
  datas=String(sized);
  datas.toCharArray(d,4);
  nh.loginfo(d);
 //reading the size of the data
 ...
 if (cmd_msg.data[0]==3){
  ...
  if (cmd_msg.data[0]==4){
    ...
    position1=stepper[2].getPosition();
    setpoint1=stepper[2].getSetpoint();
    setMP[1] = 0;
    setMP[0] = 0;
    setMP[3] = 0;
  ...
  if (cmd_msg.data[0]==5){
    setMP[1] = cmd_msg.data[1];
    setMP[2] = cmd_msg.data[2];
    setMP[3] = cmd_msg.data[3];
    setMP[4] = cmd_msg.data[4];
    }
    //set the setpoint
    for (int i=0;i<4;i++){
...
ros::Subscriber<std_msgs::Int32MultiArray> sub("distance", servo_cb);
