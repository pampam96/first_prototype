...
def talker():
    ...
    pub = rospy.Publisher('cam_distance', Int32MultiArray, queue_size=10)
    ...
    def detection_g(hsv2):
        ...
        for cnt in contours:
            ...
            roundess=((perimeter)**2)/(2*math.pi*M['m00'])
            #print("app",len(approx),"per",perimeter, M['m00'],"roundess",roundess)
            #print("roundess",roundess)
            #and perimeter>165 and perimeter<178
            if M['m00']>threshold3 and len(approx) > 8 and roundess<2.6:
                #print("app",len(approx),"per",perimeter, M['m00'],"roundess",roundess)
                #print('blob detected')
                x,y,w,h = cv2.boundingRect(cnt)
                centroid=(x+w/2,y+h/2)
    ...
    def distance(marker,reference):
        if(len(marker)<2):
            ...
            rl_pointm2 = rs.rs2_deproject_pixel_to_point(depth_intrin, [int(marker[0][3][0]),int(marker[0][3][1])], marker[0][2])
            rl_pointm3 = rs.rs2_deproject_pixel_to_point(depth_intrin, [int(marker[1][3][0]),int(marker[1][3][1])], marker[1][2])
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            while True:
                ...
                #print("nr of trackers",len(boxes),boxes)
                for i,box in enumerate(boxes):
                    ...
                    #print("centroid in boxes",centroid)
                ...
                ### iterate through trackers, draw them on frame and update my tracker list
                #print("nr of trackers b",len(boxesb),boxesb)
                ...
                if frame_count==60:
                    ...
                    #track_init[1:]=new
                    track_init.sort(key=sortSecond,reverse=True)
                    ...
                    for i,bl in enumerate(track_init):
                        ...
                        ###TRACKER PART
                        my_trackerr.append([bl[0],bl[1],bl[3],bl[4],i])
                    ...
                    for i,bl in enumerate(t_init_b):
                        ...
                        my_trackerb.append([bl[0],bl[1],bl[3],bl[4],i])
                    ...
                    #######clear the tracker first then recreate
                    trackers.clear()
                    trackers = cv2.MultiTracker_create()
                    ##############################
                    ...
                    for i,tr in enumerate(my_trackerr):
                        ...
                        if len(old_trackerc[i])>4:
                            ...
                            if distance_old>=2:
                                ...
                                ####UPDATING THE ACTUAL TRACKER KCF
                                trackers.add(tracker, frame, (tr[0][0],tr[0][1],tr[1][0],tr[1][1]))
                                ####UPDATING THE ACTUAL TRACKER KCF
                            elif distance_old<2:
                                print("no update")
                                print("tr",tr[0][0],tr[0][1],tr[1][0],tr[1][1])
                                #########WHY ARE YOU ADDING THE SAME ONE
                                ####UPDATING THE ACTUAL TRACKER KCF
                                trackers.add(tracker, frame, (tr[0][0],tr[0][1],tr[1][0],tr[1][1]))
                                ####UPDATING THE ACTUAL TRACKER KCF
                    ...
                    trackers_b = cv2.MultiTracker_create()
                    ##############################
                    print("length",len(my_trackerb),my_trackerb)
                    for i,tb in enumerate(my_trackerb):
                        ...
                        if len(old_tbc[i])>4:
                            ...
                            if distance_old>=2:
                                print("update")
                                print("tb",tb[0][0],tb[0][1],tb[1][0],tb[1][1])
                                #updating xy
                                tb[0]=old_tbc[i][0]
                                #updating centroid
                                tb[3]=old_tbc[i][4]
                                ...
                                ####UPDATING THE ACTUAL TRACKER KCF
                                trackers_b.add(tracker, frame, (tb[0][0],tb[0][1],tb[1][0],tb[1][1]))
                                ####UPDATING THE ACTUAL TRACKER KCF
                            elif distance_old<2:
                                print("no update")
                                print("tb",tb[0][0],tb[0][1],tb[1][0],tb[1][1])
                ...
                #print('init_calibration',init_calibration)
                ##here we skip the green init_calibration
